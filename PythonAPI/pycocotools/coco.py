...
#  getImgIds  - Get img ids that satisfy given filter conditions.  #  loadAnns   - Load anns with the specified ids.
    ...
    def loadNumpyAnnotations(self, data):
        ...
        for i in range(N):
            ...
            ann += [{
                ...
                'bbox'  : [ int(data[i, 1]), int(data[i, 2]), int(data[i, 3]), int(data[i, 4]) ],
    ...
    def createAnnNumpy(self, data_dir, typ):
        ...
        for fname in filenames:
            ...
            for objects in root.findall('object'):
                ...
                h = int(ymax) - int(ymin)
                score = 1.0
                result.append([image_id,xmin,ymin,w,h,score,categories[label]])
            width = root.find('size').find('width').text
            height = root.find('size').find('height').text
            file_name = root.find('filename').text
            self.dataset['images'].append({'id':image_id, 'file_name':file_name, 'width':width, 'height':height})
            image_id += 1
        self.createIndex()
